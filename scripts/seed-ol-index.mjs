/**
 * Optional: seed Open Library local index without running the app.
 * Usage: node scripts/seed-ol-index.mjs "евгений онегин" "1984"
 */
...
if (!queries.length) {
  console.error("Usage: node scripts/seed-ol-index.mjs <query> [query...]");
  process.exit(1);
}
...
for (const query of queries) {
  const url = `https://openlibrary.org/search.json?q=${encodeURIComponent(query)}&limit=20&fields=key,title,author_name,first_publish_year,edition_count,cover_i,language,subject`;
